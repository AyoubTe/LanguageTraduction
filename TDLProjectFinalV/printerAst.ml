...
struct
  ...
  let rec string_of_expression e =
    match e with
    | AppelFonction (n, le) -> 
        "call "^n^"("^(List.fold_right (fun i tq -> (string_of_expression i)^tq) le "")^") "
    | Booleen b -> 
        if b then "true " else "false "
    | Entier i -> 
        (string_of_int i)^" "
    | Unaire (op, e1) -> 
        (string_of_unaire op) ^ (string_of_expression e1)^" "
    | Binaire (b, e1, e2) -> 
        begin
          match b with
    ...
    | Affectable a -> 
        "affectable("^(string_of_affectable a)^") "
    | Adresse n -> 
        "&"^n^" "
    | New t -> 
        "new "^(string_of_type t)^" "
    | Null -> 
        "null "
  ...
  let rec string_of_instruction i =
    match i with
    | Declaration (t, n, e) -> 
        "Declaration  : "^(string_of_type t)^" "^n^" = "^(string_of_expression e)^"\n"
    | VarLStatic(t, n, e) -> 
        "VarLStatic : STATIC"^(string_of_type t)^" "^n^" = "^(string_of_expression e)^"\n"
    | Affectation (a, e) -> 
        "Affectation  : "^(string_of_affectable a)^" = "^(string_of_expression e)^"\n"
    | Constante (n, i) -> 
        "Constante  : "^n^" = "^(string_of_int i)^"\n"
    | Affichage e -> 
        "Affichage  : "^(string_of_expression e)^"\n"
    ...
    | Retour e -> 
        "Retour  : RETURN "^(string_of_expression e)^"\n"
  ...
  let string_of_var (VarGlobal(t, n, e)) = "STATIC "^(string_of_type t)^" "^n^" "^(string_of_expression e)^"\n"

  (* Conversion d'un programme Rat *)
  let string_of_programme (Programme (vars, fonctions, instruction)) =
    (List.fold_right (fun f tq -> (string_of_var f)^tq) vars "")^
