...
struct
  ...
  let rec analyser_instruction i = 
    ...
    | Ast.AstPlacement.VarLStatic (info, _)-> 
      begin
      match Tds.info_ast_to_info info with
      | InfoVar (n,_,d,r) -> [(n,(d,r))]
      | _ -> []
...
let analyser_param (info, _) =
  ...
  let analyser_var (Ast.AstPlacement.VarGlobal(info, _)) =
    match Tds.info_ast_to_info info with
    | InfoVar (n,_,d,r) -> [(n,(d,r))]
    | _ -> []
  ...
  let analyser (Ast.AstPlacement.Programme (vars, fonctions, (prog,_))) =
    ("main", List.flatten (List.map (analyser_instruction) prog))::("varg", List.flatten (List.map (analyser_var) vars))::(List.flatten (List.map (analyser_fonction) fonctions))
