...
type info =
  ...
  | InfoVarSL of string * typ * int * string * bool
...
let string_of_info info =
  ...
  | InfoFun (n,t,tp) -> "Fonction "^n^" : "^(List.fold_right (fun (elt, _) tq -> if tq = "" then (string_of_type elt) else (string_of_type elt)^" * "^tq) tp "" )^" -> "^(string_of_type t)
  | InfoVarSL(n,t,dep,reg,_)-> "VariableLocaleStatique "^n^" : "^(string_of_type t)^" "^(string_of_int dep)^"["^reg^"]"
...
let modifier_type_variable t i =
    ...
    | InfoVarSL(n,_, dep,reg,init) -> i:= InfoVarSL(n,t, dep,reg,init)
...
let%test _ = 
  ...
  let ia = info_to_info_ast info in
  modifier_type_fonction Rat [(Int, false) ; (Int, true)] ia;
  match info_ast_to_info ia with
  | InfoFun ("f", Rat, [(Int, false) ; (Int, true)]) -> true
  | _ -> false
...
(* Modifie l'emplacement (dépl, registre) si c'est une InfoVar, ne fait rien sinon *)
 let modifier_adresse_variable d b i =
     ...
     | InfoVarSL(n,t, _,_,init) -> i:= InfoVarSL(n,t,d,b,init)
...
let setInit i =
  match !i with
  | InfoVarSL(n,t,d,r,_) -> i:=InfoVarSL(n,t,d,r,true)
  | _ -> failwith "Erreur c'est pas une var st"
