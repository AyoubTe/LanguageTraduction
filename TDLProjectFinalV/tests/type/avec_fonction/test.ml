...
let%test_unit "testPointeurs1" = 
  try 
    let _ = compiler   (pathFichiersRat^"testPointeurs1.rat")
    in raise ErreurNonDetectee
  with
  | TypeInattendu(Int,Bool) -> ()

let%test_unit "testAppel14.rat" = 
  let _ = compiler   (pathFichiersRat^"testAppel14.rat") in ()

let%test_unit "testAppel15" = 
  try 
    let _ = compiler   (pathFichiersRat^"testAppel15.rat")
    in raise ErreurNonDetectee
  with
  | TypesParametresInattendus ([Bool], [Int; Int; Int]) -> ()

let%test_unit "testOkStaticAndDefault.rat" = 
  let _ = compiler   (pathFichiersRat^"testOkStaticAndDefault.rat") in ()

let%test_unit "testErreurStaticAndDefault" = 
  try 
    let _ = compiler   (pathFichiersRat^"testErreurStaticAndDefault.rat")
    in raise ErreurNonDetectee
  with
  | DoubleDeclaration("x") -> ()


let%test_unit "testValPDefaut.rat" = 
  let _ = compiler   (pathFichiersRat^"testValPDefaut.rat") in ()



let%test_unit "testVarSLErreur.rat" = 
    try 
    let _ = compiler   (pathFichiersRat^"testVarSLErreur.rat")
    in raise ErreurNonDetectee
    with
    | TypeInattendu(Int, Bool) -> ()

let%test_unit "testVarSL.rat" = 
  let _ = compiler   (pathFichiersRat^"testVarSL.rat") in ()

let%test_unit "testPointeurFonction" = 
  let _ = compiler   (pathFichiersRat^"testPointeurFonction.rat") in ()
