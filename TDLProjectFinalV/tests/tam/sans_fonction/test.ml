...
let%expect_test "complique" =
  ...
  (* tests améliortaions*)
let%expect_test "testDerefValide" =
  runtam (pathFichiersRat^"testDerefValide.rat");
  [%expect{| 8 |}]


let%expect_test "testVarG" =
  runtam (pathFichiersRat^"testVarG.rat");
  [%expect{| 20 |}]

let%expect_test "testPointeurs" =
  runtam (pathFichiersRat^"testPointeurs.rat");
  [%expect{| 423 |}]
